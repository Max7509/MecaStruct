...
def print_resultats(
    ...
    if save_csv:
        ...
        with barres_csv.open("w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["bar_id", "x1", "y1", "x2", "y2", "length", "N"])
            for i, b in enumerate(barres):
                p1 = b.n1.pos
                p2 = b.n2.pos
                writer.writerow([b.id, p1[0], p1[1], p2[0], p2[1], b.len, float(N[i])])

        react_csv = out_path / "reactions.csv"
        with react_csv.open("w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["node_id", "ux_fixed", "uy_fixed", "Rx", "Ry"])
            for r in reactions:
                writer.writerow([r["node_id"], r["ux_fixed"], r["uy_fixed"], r["Rx"], r["Ry"]])

        print(f"CSV écrits dans: {out_path}")
    ...
    if save_plots or show_plots:
        ...
        traction = mlines.Line2D([], [], color="C3", label="Traction (N>0)")
        compression = mlines.Line2D([], [], color="C0", label="Compression (N<0)")
